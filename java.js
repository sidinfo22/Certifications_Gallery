...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const itemWidth = items[0].offsetWidth + parseInt(getComputedStyle(carousel).gap); // Width + gap
    ...
    function infiniteScroll() {
        scrollPosition += 1; // Adjust scrolling speed here (higher = faster)

        // Reset scroll position when reaching the end
        if (scrollPosition >= carousel.scrollWidth / 2) {
            scrollPosition = 0; // Reset to the start of the scroll
        }

        // Apply scroll position
        carousel.scrollLeft = scrollPosition;

        requestAnimationFrame(infiniteScroll); // Smooth continuous animation
    }
